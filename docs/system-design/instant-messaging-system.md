...
如果用 Redis `INCR` 先拿 seq，再写数据库，数据库写入失败会留下永久 seq 空洞。IM 系统可以接受空洞，但客户端必须有 gap 处理策略：发现缺号后短暂等待、主动补拉，超过超时仍不存在就跳过该 seq。若业务要求会话内 seq 严格无空洞，应在数据库事务里分配 seq，或使用能和写入结果绑定确认的序号服务。
