...
- **临时故障**：短时间可能恢复的问题，比如网络抖动、连接重置、短暂 503。
- **永久失败**：重试也不会好的问题，比如参数错误、权限错误、库存不足。
- **最大尝试次数**：最多试几次，避免无限重试。
- **退避 backoff**：失败后等一段时间再重试。
- **jitter**：等待时间加随机值，避免所有客户端同时重试。
- **重试预算**：重试也必须在入口请求时间预算内完成。

读这篇时先记住：重试只能修复临时故障，不能修复业务错误。写操作重试前必须先考虑幂等。
...
重试常用于调用下游服务、第三方 API、数据库瞬时错误和 MQ 发布失败。系统设计时，要说明哪些错误可重试，哪些错误直接失败，以及重试是否会造成重复副作用。

重试通常和超时、幂等、限流、熔断一起出现。没有超时的重试会卡住资源，没有幂等的写重试会重复扣款或重复下单，没有 jitter 的重试会形成二次流量尖峰。
