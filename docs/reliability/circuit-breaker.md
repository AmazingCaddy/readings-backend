...
- **熔断**：发现下游持续失败后，暂时不再调用它，直接快速失败。
- **降级**：完整结果拿不到时，返回一个可接受的低成本结果。
- **Fallback**：降级返回的备用逻辑，比如默认值、旧值、隐藏非核心模块。
- **Closed**：熔断器正常放行请求。
- **Open**：熔断器打开，请求直接失败或走 fallback。
- **HalfOpen**：冷却后放少量请求试探下游是否恢复。
- **级联故障**：一个下游故障拖慢调用方，再拖慢更上游，故障层层扩散。

读这篇时先记住：超时是“别等太久”，熔断是“暂时别再调用坏下游”，降级是“给用户一个还能接受的结果”。
...
熔断与降级常用于商品详情依赖推荐、优惠券、评论、库存展示等非核心依赖。下游慢时，不让整个页面一起慢，而是隐藏或返回默认结果。

系统设计时，要区分强依赖和弱依赖。支付扣款、库存最终扣减这类强一致动作不能随便降级；推荐、营销、展示类模块更适合降级。
