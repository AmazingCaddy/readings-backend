...
- **高并发下单**：很多用户同时抢有限库存，系统要保证不超卖、不重复下单。
- **入口限流**：在请求进入核心逻辑前先挡掉一部分流量。
- **库存预扣**：先在 Redis 中扣一个资格，后台再创建订单。
- **异步下单**：API 先返回处理中，worker 从 MQ 里创建订单。
- **幂等表**：记录用户某次下单请求，防止重复提交。
- **补偿任务**：某一步失败后，后台把库存、订单或消息状态修正回来。
- **全链路压测**：入口、Redis、MQ、worker、数据库都参与压测，而不是只压一个接口。

读这篇时先记住：高并发下单不是单点技术，而是“入口挡流量、缓存抢资格、队列削峰、数据库做最终确认、失败可补偿”的组合。
...
这是前面文章的综合练习。你可以把它当成学习路线的阶段性项目：先用数据库事务做正确版本，再逐步加入 Redis、MQ、幂等、限流、观测和补偿。

系统设计面试里，讲高并发下单时不要只说“Redis 扣库存”。要讲清楚请求如何被限流，库存预扣成功后如何入队，订单如何最终落库，失败如何回补，用户如何查询最终结果。

## 术语回看

- [削峰](../system-design/glossary.md#削峰)
- [预扣库存](../system-design/glossary.md#预扣库存)
