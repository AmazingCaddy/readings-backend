...
- **日志**：一条条事件记录，适合看某次请求发生了什么。
- **指标**：可聚合的数字，比如 QPS、错误率、P99。
- **链路追踪 Trace**：一次请求跨多个服务的完整路径。
- **Span**：Trace 里的一段操作，比如一次 Redis 查询或一次 DB 查询。
- **Trace ID**：把日志、指标样本和 trace 串起来的请求 ID。
- **标签基数**：指标标签的取值数量。用户 ID 这种高基数字段不适合做指标标签。

读这篇时先记住：指标告诉你“哪里异常”，trace 告诉你“慢在哪段”，日志告诉你“这次具体发生了什么”。
...
可观测性不是上线后的附加项，而是系统设计的一部分。每个核心接口都应该有请求量、错误率、延迟分布、下游耗时、关键业务状态变化日志和 trace。

排查时常见路径是：先看指标发现 P99 或错误率异常，再点到慢 trace，看慢在 Redis、DB 还是 RPC，最后用 trace id 查日志确认请求参数和错误上下文。
