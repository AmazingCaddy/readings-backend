...
- **客户端总耗时**：用户从发起请求到收到响应的完整等待时间，包含网络、网关、后端和客户端下载处理。
- **服务端处理耗时**：后端应用实际处理请求的时间，常见于 access log 或 trace 根 span。
...
用户看到慢，不等于后端业务代码慢。排查时先对比客户端总耗时、网关耗时和后端 access log 里的服务端处理耗时。如果客户端耗时 2 s，但后端处理只有 80 ms，优先怀疑 DNS、TCP/TLS、弱网、网关前后传输、响应下载或客户端处理；如果后端处理本身接近 2 s，再继续看 trace 里的应用、Redis、MySQL、连接池、锁等待和下游 RPC。

```mermaid
flowchart TD
    A[User reports slow request] --> B[Compare client timing and server timing]
    B --> C{Server processing also slow?}
    C -->|No| D[Check network, gateway, response size, client handling]
    C -->|Yes| E[Open trace by trace id]
    E --> F[Find longest span]
    F --> G[Drill into dependency, pool wait, lock wait, retry]
```
...
- 客户端耗时和服务端耗时没有分开看，把网络或入口链路问题误判成后端 SQL 慢。
