...
- **连接**：应用和数据库、Redis、下游服务之间的一条网络通道。
- **连接池**：提前准备并复用一批连接，避免每次请求都重新建立连接。
- **借连接 / 还连接**：请求使用前从池里取连接，用完必须归还。
...
连接池是排查接口慢的高频入口。很多 P99 问题不是 SQL 本身慢，而是请求在等待连接。系统设计和压测时，要同时看入口 QPS、连接池等待、查询耗时和下游容量。

配置连接池时要按“实例数 × 每实例连接数”估算总连接，不能只看单个服务实例。池等待超时也要小于入口请求预算，否则请求已经超时了，后台还在等连接。
