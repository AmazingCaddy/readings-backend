...
- **消费失败**：消费者拿到消息后处理出错，比如数据库超时或消息格式错误。
- **临时错误**：过一会儿可能恢复的错误，比如网络抖动、限流、数据库短暂不可用。
- **永久错误**：重试也不会好的错误，比如消息缺字段、业务状态不允许。
- **退避**：失败后不要马上重试，而是等一段时间再试。
- **DLQ / 死信队列**：多次失败或不可重试的消息被放到隔离队列里等待处理。
- **重放**：修复问题后，把 DLQ 里的消息重新投回正常消费链路。

读这篇时先记住：重试是为了处理临时故障，DLQ 是为了隔离坏消息。两者都需要告警和人工/自动处理流程。
...
重试和 DLQ 是 MQ 系统的安全阀。订单事件、支付事件、库存事件都可能消费失败，不能让一条坏消息无限卡住整个分区或队列。

系统设计时，要说明哪些错误可以重试，最大重试几次，退避时间怎么设置，什么时候进 DLQ，DLQ 如何告警、查看和限速重放。消费者也必须幂等，因为重试和重放都会带来重复处理。
