...
- **大量 key 同时失效**：很多缓存使用相同 TTL，一起过期，数据库突然承接大量回源。
- **缓存整体不可用**：Redis 集群故障、网络分区、主从切换，导致本来能命中的请求都失败。
- **TTL 抖动**：给过期时间加随机值，把集中失效打散。
- **缓存预热**：活动开始前先把热点数据写进缓存，避免冷启动时所有请求直接查库。
- **旧值返回**：缓存过期或刷新失败时，先返回旧数据，后台再刷新，保护用户体验和数据库。

读这篇时可以把雪崩想成“缓存这层防线大面积失效”。它比击穿更危险，因为影响的不是一个 key，而是一批 key 或整个缓存层。
...
缓存雪崩常见于活动页、商品批量导入、配置批量刷新、服务重启预热不足、Redis 故障切换。系统设计时，如果一批 key 都设置成同样的 30 分钟 TTL，就要想到 30 分钟后它们可能一起回源。

典型防护是组合使用：TTL 抖动打散过期时间，核心数据提前预热，Redis 故障时限制数据库回源，允许旧值返回的接口使用 stale-while-revalidate。雪崩治理重点是保护数据库和下游，不是保证缓存永远不失效。
