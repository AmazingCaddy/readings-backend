...
### 5. 热点 key 更新时避免直接物理删除

普通 Cache-Aside 写路径常用“更新数据库后删除缓存”，但热点 key 直接删除会制造缓存空窗。下一波读请求会同时 miss，如果没有互斥回源或逻辑过期保护，就会触发击穿。热点 key 的更新更适合主动刷新缓存、逻辑过期保留旧值并异步刷新，或者在必须删除时配合互斥回源和回源限流。对于首页配置、热门榜单这类超热点低频更新数据，可以组合使用逻辑过期和写后主动刷新：读路径始终有旧值可返回，写路径尽快把新值推入 Redis。
...
- 热点 key 更新后直接删除 Redis，制造缓存空窗并触发并发回源。
...
- 商品或推荐配置更新时主动刷新热点缓存，避免直接删除热点 key。
...
- 逻辑过期适合什么场景，代价是什么？
- 热点 key 更新时，为什么不能简单更新数据库后删除 Redis？
- 应该监控哪些指标来发现击穿风险？
...
常见答案是组合拳：热点 key 预热，TTL 加随机抖动，miss 后用互斥回源，允许短暂旧值的场景使用逻辑过期。热点 key 更新时也要避免直接物理删除，可以主动刷新或保留旧值异步刷新。这样数据库不会因为单个热点 key 过期或更新被打满。
