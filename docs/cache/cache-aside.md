...
- **回源保护**：Redis miss 或故障时，限制进入数据库的流量，避免缓存故障扩大成数据库故障。
- **缓存 key 维度**：key 要和数据维度一致，例如商品基础信息用商品维度，用户收藏状态用用户-商品维度。
...
数据库擅长持久化和事务，但不适合承受所有高频读流量。商品详情、用户展示资料、配置项、菜单配置、库存摘要、首页推荐等读多写少场景，如果每次都查数据库，会消耗连接池、CPU、IO 和锁资源。
...
| 回源限流 | Redis 故障或大量 miss 时保护数据库 | 部分请求需要快速失败或降级 |
| key 维度拆分 | 提高命中率，避免不同维度数据互相污染 | 接口层需要组合多个数据源结果 |
...
版本号可以降低旧值回填风险。缓存 value 不只保存业务数据，也保存 `version` 或 `updated_at`。读请求回填缓存前，只有在缓存不存在或缓存版本不高于当前数据库版本时才写入，避免旧版本覆盖新版本。但如果写路径只是删除缓存，没有写入新版本标记，那么旧读请求仍可能在缓存为空时写回旧值；因此版本控制通常要配合新版本标记、延迟双删或 CDC 失效一起使用。
...
缓存 key 的维度要和数据本身一致。商品基础详情是商品维度，适合 `product:{id}`；用户是否收藏商品是用户-商品维度，适合 `favorite:{userId}:{productId}` 或单独批量查询后在接口层合并。不要把用户态字段塞进商品基础详情缓存，否则要么把某个用户的状态错误返回给其他用户，要么把 key 拆成 `product:{id}:user:{userId}`，导致命中率下降、缓存膨胀和失效逻辑变复杂。

### 7. Redis 故障时保护数据库

Redis 的吞吐量通常远高于数据库。Redis 故障、超时或大面积 miss 时，不能让所有流量直接回源数据库，否则数据库连接池、CPU 和 IO 可能被瞬间打满。工程上要根据数据库安全吞吐做回源限流，只放行一部分请求；其他请求可以快速失败、返回降级字段、使用本地缓存或静态快照。缓存客户端也要设置短超时和熔断，避免请求长时间卡在 Redis 调用上。

### 8. 命中率低时先解释为什么没命中

如果商品详情接入 Cache-Aside 后 Redis 命中率仍然很低，不要只看“有没有加缓存”。先检查同一个业务对象是否被拼成了多个 key，例如参数顺序、渠道、语言、用户态、实验版本或租户字段导致 key 分裂；再看 TTL 是否太短、miss 后是否真的写回 Redis、是否有大量不存在 id 需要空值缓存、Redis 是否发生超时或 key 淘汰、缓存容量是否足够。命中率排查要和 DB QPS、DB P99、Redis `keyspace_hits/misses`、GET/SET 延迟一起看。
...
- 把用户态字段混进公共对象缓存，导致 key 维度膨胀或跨用户返回错误数据。
- 命中率低时只调 TTL，不排查 key 分裂、写回失败、Redis 淘汰和不存在 key 流量。
...
- Redis 故障时，如何按数据库容量限制回源流量？
- 缓存命中率低时，如何排查 key 设计、TTL、写回、淘汰和不存在 key？
- 为什么用户态字段不应该混进商品基础详情缓存？
- 单个热点 key 过期时，如何避免大量请求同时回源数据库？
- 应该监控哪些指标判断缓存是否真的保护了数据库？

## 这篇文章在系统里怎么用

Cache-Aside 通常是你接入 Redis 的第一种模式。商品详情、用户展示资料、配置项、菜单配置这类读多写少的数据，都可以先从这个模式开始。

在系统设计题里，提到 Cache-Aside 时不要只说“加 Redis”。要继续说明：缓存 miss 后如何回源、写操作为什么删除缓存、TTL 怎么设置、key 维度如何设计、热点 key 过期时怎么防击穿、缓存不可用时数据库能不能扛住。
