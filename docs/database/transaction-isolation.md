...
- **事务**：一组数据库操作，要么都成功，要么都回滚。
- **提交 commit**：事务完成，修改正式生效。
- **回滚 rollback**：事务失败，撤销已经做的修改。
- **隔离级别**：多个事务同时执行时，彼此能看到多少中间状态。
- **脏读**：读到了别人还没提交的数据。
- **不可重复读**：同一事务里两次读同一行，结果不一样。
- **幻读**：同一事务里两次范围查询，第二次多出或少了行。
- **MVCC**：数据库用多版本数据让读写尽量不互相阻塞。
- **affected rows**：写 SQL 实际影响的行数，常用于判断条件更新是否成功。
- **唯一约束**：数据库层保证某个业务维度只能存在一条记录，是防重复写入的重要兜底。

读这篇时先记住：事务不是自动解决所有并发问题。隔离级别、锁、唯一约束和条件更新要一起设计。
...
执行条件更新后必须检查 `affected rows`。在扣库存场景中，`UPDATE inventory SET stock = stock - 1 WHERE sku_id = ? AND stock > 0` 返回 1 表示扣减成功；返回 0 通常表示库存不足、记录不存在或条件已经不成立。业务应该回滚并返回售罄/失败，不能忽略影响行数继续创建订单。

### 3. 防重复写入要用唯一约束兜底

应用层“先查有没有订单，再插入”在高并发下不安全。两个请求可能同时查不到，然后都插入成功。防重复下单、幂等请求、活动报名这类场景要在数据库层建立唯一约束，例如 `UNIQUE(user_id, activity_id)` 或 `UNIQUE(idempotency_key)`。唯一键冲突时，业务可以返回已存在结果或按幂等语义返回同一订单。
...
重试必须重试整个事务，而不是只重试最后一条 SQL。因为事务前面的读取、判断和业务状态可能已经失效。常见做法是最多重试 1 到 3 次，配合短暂 backoff 和日志指标；超过次数后返回失败或提示稍后再试。
...
- 条件更新后不检查 affected rows，扣库存失败后仍然继续创建订单。
- 死锁或序列化失败后无限重试，造成更大压力。
- 死锁或序列化失败后只重试最后一条 SQL，没有重新执行整个事务。
- 在只读报表和核心交易上使用同一事务策略。
...
- 为什么“先读后写”在高并发下危险？
- 条件更新为什么能防止库存超卖？
- `affected rows = 0` 在条件扣库存里通常代表什么？
- 唯一索引和事务隔离分别解决什么问题？
- 为什么防重复下单不能只靠应用层先查再插入？
- 为什么事务里不能做慢 IO？
- 哪些数据库错误适合有限重试？

## 这篇文章在系统里怎么用

事务隔离常用于库存、余额、支付、订单状态这类不能出错的写路径。系统设计时，如果有“先查再改”的逻辑，就要思考并发下两个请求同时查到旧值怎么办。

常见做法不是盲目提高到最高隔离级别，而是用条件更新、唯一索引、行锁和短事务缩小风险。隔离级别决定你能看到什么，业务约束决定什么更新是合法的。
