...
- **低选择性字段**：取值很少、过滤能力弱的字段，例如 `is_deleted`、布尔状态，通常不适合单独建索引。
...
走了索引不代表 SQL 一定快。还要看扫描了多少索引记录、过滤后剩多少行、是否大量回表、是否还有额外排序。低选择性索引或范围过大的查询，即使命中了 `key`，也可能扫描几十万甚至几百万条记录。
...
`is_deleted`、`true/false` 状态这类字段选择性很低，单独建索引往往过滤不出小集合。例如大部分订单都是 `is_deleted = false`，数据库走这个索引仍然会命中大量行，收益有限。更常见的做法是把它放进符合业务访问模式的复合索引里，例如 `(user_id, is_deleted, created_at DESC)` 或 `(tenant_id, status, is_deleted, created_at DESC)`，配合高选择性字段一起缩小扫描范围。

### 6. 覆盖索引要克制使用

覆盖索引能避免回表，但不是把整张表复制进索引。为了覆盖查询把大字段、JSON、备注、地址等都塞进索引，会让索引变大，降低缓存效率，并增加 insert、update、delete 的维护成本。覆盖索引更适合高频、返回字段少、字段较小的核心查询。
...
- 给 `is_deleted` 这类低选择性字段单独建索引，实际过滤效果很差。
- 为了覆盖索引塞入过多返回字段，让索引膨胀并拖慢写入。
...
- 为什么复合索引的字段顺序很重要？
- 为什么 `is_deleted` 这类低选择性字段通常不适合单独建索引？
- 覆盖索引有什么好处？为什么不能无限塞字段？
- `EXPLAIN` 中应该重点看哪些字段？
- 为什么“走了索引”不等于查询一定快？
