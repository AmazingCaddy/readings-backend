...
- **长事务**：事务持续时间过长，锁和连接被占用更久，容易放大 P99。
- **锁等待时间**：SQL 真正执行前等待别人释放锁的时间，和 SQL execution time 不是一回事。
...
锁等待变高会间接推高连接池等待。一个事务在等待锁时通常仍然占用数据库连接，请求耗时变长后，连接归还速度下降；在入口 QPS 不变时，连接池 active 更容易打满，后续请求开始 pool wait。因此 trace 里如果同时看到 `lock wait` 和 `connection pool wait` 高，但 `SQL execution` 很短，不要第一反应调大连接池。优先查谁持有锁、事务持续多久、是否有批量更新、热点行、事务内 HTTP 调用或缺索引范围更新。

### 7. 批量更新要缩小锁范围和事务时间

后台任务例如“把超时未支付订单改成 EXPIRED”很容易制造锁等待。如果 SQL 缺少合适索引，数据库会扫描大量订单；如果一次更新太多行，事务会长时间持有大量排他锁，和下单、支付回调等在线请求互相阻塞。常见做法是给筛选条件建立组合索引，按主键或时间窗口分批更新，每批快速提交，并限制后台任务并发。
...
- trace 已经显示 lock wait 最高，却继续优化 SQL execution 或大幅调连接池。
- 后台批量 UPDATE 一次锁住大量行，影响在线下单、支付回调等请求。
...
## 完整案例：批量过期订单导致下单 P99 升高

### 场景

下单接口 P99 突然升高，trace 显示 `lock wait = 1800 ms`、`SQL execution = 40 ms`、`connection pool wait = 600 ms`。最近上线了一个后台任务，批量把超时未支付订单更新为 `EXPIRED`。

### 排查

先按 trace 判断瓶颈是锁等待，而不是 SQL 执行慢。再关联最近变更，检查批量过期订单的 UPDATE 是否缺少 `(status, expire_at)`、`(status, created_at)` 或更符合业务条件的索引，是否一次更新太多行，事务是否过长，是否和下单或支付回调争抢同一批订单行。

### 修复方向

- 给批量任务的过滤条件补合适组合索引，减少扫描和锁范围。
- 按主键或时间窗口分批更新，每批尽快提交。
- 限制后台任务并发，避开交易高峰。
- 观察 lock wait、transaction duration、deadlock count、pool wait 和订单接口 P99 是否下降。
...
- lock wait 高为什么可能同时推高 connection pool wait？
- 批量 UPDATE 导致线上接口变慢时，应该如何排查和改造？
- 应该监控哪些锁相关指标？
